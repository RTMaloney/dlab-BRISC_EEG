function PlayMoviesDemo_RM(moviename, backgroundMaskOut, tolerance, pixelFormat, maxThreads)
...
try
    ...
    [win, winRect] = PsychImaging('OpenWindow', screen, [0.5, 0.5, 0.5]);
    ...
    if isempty(strfind(moviename, 'http'))
        ...
        if isempty(moviefiles)
            ...
            moviefiles(1).name = ['C:\Users\BRISC\Videos\The Wiggles - Romp Bomp A Stomp_edit.mp4'];
            %moviefiles(1).name = ['C:\Users\BRISC\Videos\ComplexMotionEbbing180Deg_test1.avi'];
            %moviefiles(1).name = ['C:\Users\BRISC\Videos\Star Wars Episode VII The Force Awakens (2015) 720p BluRay x265 HEVC SUJAIDR.mkv'];
        else
    ...
    while (abortit<2)
        ...
        [movie movieduration fps imgw imgh] = Screen('OpenMovie', win, moviename, ...
            [], ... % preloadsecs: was empty anyway (RM)
            [], ... %asyc (?)...empty here already
            2, ... % specialFlags: to get rid of sound (may be faster) RM.
            6, ... % pixelFormat, 5 or 6 meant to be better..(see above), RM
            []); % maxThreads; set to default anyway

        fprintf('Movie: %s  : %f seconds duration, %f fps, w x h = %i x %i...\n', moviename, movieduration, fps, imgw, imgh);
       
        
        % RM: Determine new rect based on movie resolution, to make the movie smaller on screen
        %movieRect = CenterRectOnPointd ([0 0 (imgw/2) (imgh/2)], winRect(3)/2, winRect(4)/2);
        ...
        %Screen('PlayMovie', movie, rate, 1, 1); %RM: set volume to zero
        ...
        while 1
            ...
            if ((abs(rate)>0) && (imgw>0) && (imgh>0))
                ...
                %Screen('DrawTexture', win, tex, [], movieRect, [], [], [], [], shader); %RM: added movieRect
                Screen('DrawTexture', win, tex, []); %, [], [], [], [], shader); %RM: added movieRect
